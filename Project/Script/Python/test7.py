...
model = Sequential()
model.add(Conv2D(2, kernel_size=3, use_bias = False, input_shape=(32,32,1))) #30x30x32
model.add(Conv2D(2, kernel_size = 3, use_bias = False, activation = 'relu'))  #28x28x32
model.add(MaxPooling2D(pool_size = (2, 2), strides = (2, 2), padding = 'valid')) #14x14x32
model.add(Conv2D(5, kernel_size=3, use_bias = False)) #12x12x64
model.add(Conv2D(5, kernel_size = 3, use_bias = False, activation = 'relu'))  #10x10x64
model.add(MaxPooling2D(pool_size = (2, 2), strides = (2, 2), padding = 'valid')) #5x5x64
model.add(Conv2D(10, kernel_size=3, use_bias = False)) #3x3x128
model.add(Conv2D(10, kernel_size = 3, use_bias = False, activation = 'relu'))  #1x1x128
